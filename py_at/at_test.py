...
class at_test:
    ...
    def on_order(self, stra, order):
        ...
        print('{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}'.format(
            len(p.Orders), stra.Bars[0].D, _order.Direction, _order.Offset,
            _order.Price, _order.Volume, _order.Remark))
        ...
        if self.real:
            ...
            if _order.Offset != Offset.Open:
                key = '{0}_{1}'.format(
                    _order.Instrument,
                    int(DirectionType.Sell if _order.Direction == Direction.Buy
                        else DirectionType.Buy))
                ...
                if not pf or pf.Position <= 0:
                    ...
                    if instField.ExchangeID == 'SHFE':
                        ...
                        if tdClose > 0:
                            self.t.ReqOrderInsert(_order.Instrument, dire,
                                                  OffsetFlagType.CloseToday,
                                                  _order.Price, tdClose,
                                                  OrderType.Limit, 100)
                            volClose -= tdClose
                    if volClose > 0:
                        self.t.ReqOrderInsert(
                            _order.Instrument, dire, OffsetFlagType.Close,
                            _order.Price, volClose, OrderType.Limit, 100)
            else:
                self.t.ReqOrderInsert(stra.Instrument, dire,
                                      OffsetFlagType.Open, _order.Price,
    ...
    def OnFrontConnected(self):
        ...
        self.t.ReqUserLogin('008109', '1', '9999')
    ...
    def OnCancel(self, pOrder=CThostFtdcOrderField):
        """"""
        print(pOrder)
    ...
    def q_OnFrontConnected(self):
        ...
        self.q.ReqUserLogin(BrokerID='9999', UserID='008109', Password='1')
