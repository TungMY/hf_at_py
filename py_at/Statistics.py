...
class Statistics():
    ...
    def CalculateProfit(self):
        ...
        for i in range(0, len(self.stra.C) - 1):
            profitLong_list.append(
                (self.stra.C[i] - avgPositionLong) * positionLong)
            profitShort_list.append(
                (avgPositionShort - self.stra.C[i]) * positionShort)
                ...
                if order.DateTime == self.stra.D[i]:
                    ...
                    if order.Offset == Offset.Open:
                        if order.Direction == Direction.Buy:
                            avgPositionLong = order.Price if positionLong == 0 else (
                                avgPositionLong * positionLong +
                                order.Price * order.Volume) / (
                                    positionLong + order.Volume)
                            positionLong += order.Volume
                        else:
                            avgPositionShort = order.Price if positionShort == 0 else (
                                avgPositionShort * positionShort +
                                order.Price * order.Volume) / (
                    ...
                    # 平仓
                        ...
                        for x in range(idx_order - 1, -1,
                            ...
                            if openOrder.Direction == openDire and openOrder.Offset == Offset.Open:
                                ...
                                if profit > 0:
                                    tradeProfitTimes += 1
                                    maxContinueTradeProfitTimes = max(
                                        maxContinueTradeProfitTimes,
                                        tradeProfitTimes)
                                    ...
                                    tradeLossTimes += 1
                                    maxContinueTradeLossTimes = max(
                                        maxContinueTradeLossTimes,
                                        tradeLossTimes)
                                ...
                                if openDire == Direction.Buy:
                                    if profit > 0:
                                        ...
                                        maxContinueTradeProfitTimesLong = max(
                                            maxContinueTradeProfitTimesLong,
                                            tradeProfitTimesLong)
                                    else:
                                        tradeProfitTimesLong = 0

                                        tradeLossTimesLong += 1
                                        maxContinueTradeLossTimesLong = max(
                                            maxContinueTradeLossTimesLong,
                                            tradeLossTimesLong)
                                else:
                                    if profit_list > 0:
                                        tradeLossTimesShort = 0
                                        ...
                                        maxContinueTradeProfitTimesShort = max(
                                            maxContinueTradeProfitTimesShort,
                                            tradeProfitTimesShort)
                                        maxLossShort = 0
                                    else:
                                        tradeProfitTimesShort = 0
                                        ...
                                        maxContinueTradeLossTimesShort = max(
                                            maxContinueTradeLossTimesShort,
                                            tradeLossTimesShort)
                                #1开1平（多开多平待完善）
                                break
        #报表
    ...
    def GetDataAndBars(self):
        ...
        for i in range(0, len(orders)):
            ...
            orders_json.append({
                "DateTime":
                ord.DateTime.replace('-', ''),
                "Direction": (0 if ord.Direction == Direction.Buy else 1),
                "Offset":
                0 if ord.Offset == Offset.Open else 1,
                "Price":
                ord.Price
            })
