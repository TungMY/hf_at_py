...
class AdapterTest:
    ...
    def on_order(self, stra, order):
        ...
        if self.real:
            ...
            if order.Offset != Offset.Open:
                key = '{0}_{1}'.format(
                    order.Instrument,
                    int(DirectionType.Sell if order.Direction == Direction.Buy
                    ...
                    if instField.ExchangeID == 'SHFE':
                        ...
                        if tdClose > 0:
                            self.t.ReqOrderInsert(
                                order.Instrument, dire, OffsetType.CloseToday,
                                order.Price, tdClose, OrderType.Limit, 100)
                            volClose -= tdClose
                    if volClose > 0:
                        self.t.ReqOrderInsert(order.Instrument, dire,
                                              OffsetType.Close, order.Price,
                                              volClose, OrderType.Limit, 100)
            else:
                self.t.ReqOrderInsert(stra.Instrument, dire, OffsetType.Open,
    ...
    def load_strategy(self):
        ...
        for path in ['strategies']:
            ...
            files = os.listdir(os.path.join(sys.path[0], '..'))
            for f in files:
